...
//	}
	...
	for ($i = 0; $i < count($_POST['vaksite_id2']); $i++) {
		$cur = $_POST['cur'][$i];
		$max = $_POST['max'][$i];
		$vaksite_id = $_POST['vaksite_id2'][$i];
		if (!check_integer($cur) || !check_integer($max) || $max <= 0 || $cur <= 0 || $cur > $max)
			mysql_query_safe("DELETE FROM ppl2vaksiteprefs WHERE ppl_id = '{$_SESSION['ppl_id']}' AND vaksite_id = '%s'", mysql_escape_safe($vaksite_id));
		else
			mysql_query_safe("INSERT INTO ppl2vaksiteprefs ( ppl_id, vaksite_id, cur, incr, max ) VALUES ( '{$_SESSION['ppl_id']}', '%s', '%s', 1, '%s' ) ON DUPLICATE KEY UPDATE cur = '%s', incr = 1, max = '%s'",
				mysql_escape_safe($vaksite_id), mysql_escape_safe($cur), mysql_escape_safe($max),
				mysql_escape_safe($cur), mysql_escape_safe($max));
