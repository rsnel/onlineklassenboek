...
SELECT login,
	...
	COUNT(IF(DATEDIFF(NOW(), timestamp) > 10 AND DATEDIFF(NOW(), timestamp) <= 30, ppl.ppl_id, NULL)) quarter,
	MAX(timestamp) quux
FROM log
...
function drawChart() {
	var data = new google.visualization.DataTable();
        ...
        data.addColumn('number', 'Langer geleden');
<? $start = 0; while (mysql_result($result, $start, 4) == 0) $start++ ?>
...
<? for ($i = $start; $i < mysql_num_rows($result); $i++) { 
	...
	echo "\tdata.setValue($i - $start, 4, ".mysql_result($result, $i, 4).");\n";
} ?>
