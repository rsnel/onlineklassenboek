...
function curl_teletop_req($ch, $url, $postdata = NULL, $additional_headers = array(), $recurse = 0) {
	...
	if (!strncasecmp($content_type, 'text/html', 9)) {
		...
		if (!$ret2 && $_SESSION['ppl_id'] == 3490) {
			...
			echo "error decoding JSON data from TeleTOP&reg; ".json_last_error().'<br>';
...
function change_in_teletop($grp2vak_id, $notitie_id, $week, $dag, $text, $sa = '') {
	...
	// hier simuleren we userinput in het form
	if (!($checkbox = $xpath->query('.//input[@name="TargetAudienceUserGroup" and @type="checkbox" and not(@disabled) and @value="'.$klas.'"]', $form)->item(0))) {
		// hebben we te maken met een cluster?, hopelijk wel
		dom_form_check_checkbox($form, $xpath, 'TargetAudience', $klas);

		// vink ook alle stamklassen aan, anders schijnen de leerlingen het niet te kunnen zien
		dom_form_check_checkboxen($form, $xpath, 'TargetAudienceUserGroup');
	} else dom_form_check_checkbox($form, $xpath, 'TargetAudienceUserGroup', $klas);
...
, mysql_escape_safe($grp2vak_id)); 
	...
	// hier simuleren we userinput in het form
	if (!($checkbox = $xpath->query('.//input[@name="TargetAudienceUserGroup" and @type="checkbox" and not(@disabled) and @value="'.$info['klas'].'"]', $form)->item(0))) {
		// hebben we te maken met een cluster?, hopelijk wel
		dom_form_check_checkbox($form, $xpath, 'TargetAudience', $info['klas']);

		// vink ook alle stamklassen aan, anders schijnen de leerlingen het niet te kunnen zien
		dom_form_check_checkboxen($form, $xpath, 'TargetAudienceUserGroup');
	} else dom_form_check_checkbox($form, $xpath, 'TargetAudienceUserGroup', $info['klas']);
	...
	// dit is het default nummer van TeleTOP&reg; we kunnen ook werken met een gespecificeerde range
	// even kijken of er een range voor deze docent/vaksite is...
	$rownr = sprint_singular("SELECT cur FROM ppl2vaksiteprefs WHERE ppl_id = '{$_SESSION['ppl_id']}' AND vaksite_id = '%s'",
		              mysql_escape_safe($info['vaksite_id']));
	if ($rownr) {
		dom_form_set_input($form, $xpath, 'RosterRowNr', $rownr);
		$roster_row_nr = $rownr;
		mysql_query_safe("UPDATE ppl2vaksiteprefs SET cur = cur + incr WHERE ppl_id = '{$_SESSION['ppl_id']}' AND vaksite_id = '%s' AND cur + incr <= max", mysql_escape_safe($info['vaksite_id']));
	}
	...
	$items = $dingies->length;
	if (!$items) regular_error($http_path.'/', (array) NULL,
	...
	// indien er meerdere regels zijn met dezelfde RosterRowNr, dan nemen we aan
	// dat de laatste regel degene is die we zojuist hebben toegevoegd

	$doc_id = preg_replace('/writeEditLink\(\'id\/(.*)\?EditDocument.*$/', '$1', $dingies->item($items - 1)->nodeValue);
